...
// Source File Name:   StrBuilder.java
    ...
    implements Cloneable
{
    ...
    public StrBuilder appendAll(Collection coll)
    {
        ...
        {
            for(Iterator it = coll.iterator(); it.hasNext(); append(it.next()));
        }
        return this;
    }
    ...
    {
        endIndex = validateRange(startIndex, endIndex);
        int insertLen = replaceStr != null ? replaceStr.length() : 0;
    ...
    {
        int searchLen = searchStr != null ? searchStr.length() : 0;
        if(searchLen > 0)
        {
...
label0:
    ...
    {
        startIndex = startIndex < size ? startIndex : size - 1;
        if(startIndex < 0)
            return -1;
