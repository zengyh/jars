...
public class MetaObjectClassExporter
{
    ...
    public static void exportJavaBean(String entityName, boolean exportSGMethod, OutputStream os, DataSource ds, String encoding)
        throws IOException
    {
        String template = getTemplateContext("com/sinitek/base/metadb/util/javaBean.template");
        EntityBean bean = new EntityBean(entityName, ds);
        int pos = bean.getInterfaceName().lastIndexOf('.');
        String interfaceName = bean.getInterfaceName().substring(pos + 1);
        String interfacePackage = bean.getInterfaceName().substring(0, pos);
        pos = bean.getClassName().lastIndexOf('.');
        String packageName = bean.getClassName().substring(0, pos);
        String implName = bean.getClassName().substring(pos - 3);
        template = template.replaceAll("\\$\\{cataloginfo\\}", bean.getCatalogInfo());
        template = template.replaceAll("\\$\\{entityinfo\\}", bean.getEntityInfo());
        template = template.replaceAll("\\$\\{interfacename\\}", interfaceName);
        entityName = entityName.trim().toUpperCase();
        ...
        if(exportSGMethod)
        {
            ...
            PropertyBean pBean;
            String returnClass;
            for(Iterator iter = PropertyBean.getProperties(entityName, ds).iterator(); iter.hasNext(); sb.append("    private ").append(returnClass).append(" ").append(StringUtils.uncapitalize(pBean.getPropName())).append(";\n\n"))
            {
                pBean = (PropertyBean)iter.next();
                returnClass = getPropertyClassName(packageName, pBean, otherImports, ds);
                sb.append("    //").append(pBean.getPropInfo()).append("\n");
            }

            template = template.replaceAll("\\$\\{propertyimpl\\}", sb.toString());
        } else
        {
            template = template.replaceAll("\\$\\{propertyimpl\\}", "");
        }
        if(exportSGMethod)
        {
            StringBuffer sb = new StringBuffer();
            for(Iterator iter = PropertyBean.getProperties(entityName, ds).iterator(); iter.hasNext(); sb.append("\n\n"))
            {
                PropertyBean pBean = (PropertyBean)iter.next();
                String returnClass = getPropertyClassName(packageName, pBean, otherImports, ds);
                sb.append("    public ").append(returnClass).append(" get").append(StringUtils.capitalize(pBean.getPropName())).append("()\n");
    ...
    private static String getTemplateContext(String templateFile)
    {
        InputStream is;
        Exception exception;
        is = Thread.currentThread().getContextClassLoader().getResourceAsStream(templateFile);
        String s;
        try
        {
