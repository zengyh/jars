    ...
    protected void readSingleConfigFile(String configFile)
    {
        ...
        if(url != null)
            try
            {
                ...
                for(Iterator iter = configInfos.iterator(); iter.hasNext();)
                {
                    IContextSupportConfigInfo config = (IContextSupportConfigInfo)iter.next();
                    ...
                    try
                    {
                        IMetaDBContextSupport proxy = MetaDBContextSupportBuilder.createProxy(config);
                        _tempInterface.put(config.getInterface(), proxy);
                        _tempName.put(config.getName(), proxy);
                    }
                    catch(Exception ex)
                    {
                        isok = false;
                        LOGGER.info((new StringBuilder()).append("createProxy [").append(config.getName()).append("] failed").toString(), ex);
                    }
                }

                proxyInterfaceMap.putAll(_tempInterface);
                proxyNameMap.putAll(_tempName);
                LOGGER.info((new StringBuilder()).append("ContextSupport\u914D\u7F6E\u6587\u4EF6[").append(configFile).append("]\u8BFB\u53D6").append(isok ? "\u6210\u529F" : "\u5931\u8D25").toString());
