    ...
    implements IMetaDBSysLoader
{
    ...
    {
        if(globalConfig.getMetaDBProperties().getProperty("datasource.strategy").equals("none"))
            return;
        ...
        MetaDBTableCreator.getInstance().createSequence(globalConfig.getDataSourceProvider().getDataSource(), new ArrayList(entityMap.values()));
        startFlag = true;
        LOGGER.debug("MetaDB\u6838\u5FC3\u521D\u59CB\u5316\u5B8C\u6BD5");
    }

    public void initOtherData(GlobalConfig globalConfig)
        throws MetaDBException
    {
        if("true".equalsIgnoreCase(globalConfig.getMongoDBProperties().getProperty("mongodb.enable")))
        {
            Properties properties = globalConfig.getMongoDBProperties();
            String host = properties.getProperty("mongodb.host");
            String port = properties.getProperty("mongodb.port");
            String db = properties.getProperty("mongodb.db");
            String user = properties.getProperty("mongodb.user");
            String password = properties.getProperty("mongodb.password");
            String readPreference = properties.getProperty("mongodb.readpreference");
            MetaDBMongoFactory.init(host, port, db, user, password, readPreference);
        }
    }

    public void reloadMongoDB(GlobalConfig globalConfig)
    {
        if("true".equalsIgnoreCase(globalConfig.getMongoDBProperties().getProperty("mongodb.enable")))
        {
            Properties properties = globalConfig.getMongoDBProperties();
