...
//            ServerDealThreadGroup, SynServerListernThread, AppErrorLogCheckThread, SynCenterServerException, 
//            SynServerConfig
    ...
    implements IStarter
{
    ...
    private static void startServer(SynServerConfig config)
    {
        ...
        throws Exception
    {
        ...
        Properties synProp;
        Exception exception;
        cfgFile = "synserverconfig.properties";
        URL configUrl = Thread.currentThread().getContextClassLoader().getResource(cfgFile);
        inputStream = null;
        synProp = new Properties();
        try
        {
            inputStream = configUrl.openStream();
            synProp.load(inputStream);
        }
        catch(Exception ex)
        {
        ...
        throw exception;
        if("true".equalsIgnoreCase(synProp.getProperty("synserver.enable", "false")))
        {
            SynServerConfig config = SynServerConfig.loadConfig(synProp);
            startServer(config);
        } else
        {
            LOGGER.warn("\u540C\u6B65\u4E2D\u5FC3\u670D\u52A1\u5668\u8BBE\u7F6E\u4E3A\u4E0D\u542F\u7528");
        }
        return;
    }
